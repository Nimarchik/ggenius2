...
export const AuthProvider = ({ children }) => {
  ...
  const fetchUser = async (token) => {
    if (!token) return;
    // http://localhost:8000/bots/user.php
    // Publish https://ggenius-api.onrender.com/bots/user.php
    try {

      const res = await fetch("http://localhost:8000/bots/user.php", {
        method: "POST",
  ...
  const refreshToken = async () => {
    if (!refresh) return;
    // http://localhost:8000/bots/refresh.php
    // Publish https://ggenius-api.onrender.com/bots/refresh.php
    try {
      const res = await fetch("http://localhost:8000/bots/refresh.php", {
        method: "POST",
