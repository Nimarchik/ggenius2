...
import { useTelegramAuth } from '../../domains/auth/hooks/useTelegramAuth';
import { useLanguage } from '../../shared/i18n/hooks/LangChange';
...
const Header = () => {
  ...
  //     // http://localhost:8000
  //     fetch(`https://ed8f-109-229-2-47.ngrok-free.app/bots/auth.php?${params.toString()}`)
  //       .then(res => res.json())
  //       .then(data => {
  //         login(data);
  ...
  useTelegramAuth({ auth, login });

  const loginViaBot = () => {
    const botUsername = 'TestAuthBotb_bot'; // TestAuthBotb_bot  || ggeniusgg_bot
    window.location.href = `https://t.me/${botUsername}?start=auth`;
  }

  return (
    <header className={style.header}>
      <div className={menu ? style.container : style.container}>
        <nav className={style.nav}>
          <Link to={`/Home?lang=${lang}`} className={style.navLogoLink}>
            <img src={logo} alt="Logo" className={style.navLogoLinkImg} fetchPriority='high' />
            <img src={menu ? logo : logoMob} alt="Logo" className={menu ? style.navLogoLinkImgMob : style.navLogoLinkImgMobDis} fetchPriority='high' />
          </Link>
            ...
            <div className={style.autho}>
              {user ? (
                <div className={style.navLinkLogin}>
                  <NavLink to={`/profile?lang=${lang}`} className={style.navLinkLoginUsers}>
          ...
          <div className={style.navMenu}>
            ...
            <ul className={style.navList}>
              {/* 'Donate' */}
              {['Home', 'About', 'Work', 'Our', 'Donate'].map((item, idx) => (
          ...
          <div className={menu ? style.navMenuActive : style.navMenuDis}>
            <div className={style.navMenuActiveDiv}>
              ...
              <div className={style.navIns}>
                ...
                <Link onClick={user ? logout : ''} className={user ? style.navLinkLoginListItemSub : style.navLinkLoginListItemSubDis}>
                  {t('navLinkLoginListItemSub')}
                </Link>
                {/* <Link to={'https://lang.me/ggenius_chat'} className={style.navActiveBtnsLink}>
                    {lang('navActiveBtnsLink')}
                  </Link> */}
              </div>
            </div>
          </div>
